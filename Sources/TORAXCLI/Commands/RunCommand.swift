// RunCommand.swift
// Command for running TORAX simulations with full Torax compatibility
...
struct RunCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            Relative paths are resolved first against the working directory, then the TORAX base directory.
    ...
    @Option(
        name: .long,
        help: "Path to configuration file (JSON). Relative paths resolved against cwd, then TORAX base."
    )
    var config: String
    ...
    private func resolveConfigPath(_ path: String) throws -> String {
        ...
        // 3. Relative to TORAX base directory
        if let basePath = findToraxBaseDirectory() {
            let toraxPath = basePath + "/" + path
            if FileManager.default.fileExists(atPath: toraxPath) {
                return toraxPath
            }
        }
    ...
    /// Find TORAX base directory by looking for characteristic directories
    private func findToraxBaseDirectory() -> String? {
        var currentPath = FileManager.default.currentDirectoryPath

        // Look up directory tree for TORAX markers
        for _ in 0..<5 {
            let examplesPath = currentPath + "/examples"
            let sourcesPath = currentPath + "/Sources/TORAX"
    ...
    private func saveResults(
        ...
        // Convert TORAX.OutputFormat to torax_cli.OutputFormat
