...
// Tests for ToraxConfigReader with swift-configuration integration

import Testing
import Foundation
@testable import TORAX

#if canImport(TORAXCLI)
import TORAXCLI

@Suite("ToraxConfigReader Integration Tests")
struct ToraxConfigReaderTests {
    ...
    func testLoadMinimalConfig() async throws {
        ...
        let reader = try await ToraxConfigReader.create(
            jsonPath: configPath,
            cliOverrides: [:]
        )
    ...
    func testEnvironmentVariables() async throws {
        ...
        setenv("TORAX_MESH_NCELLS", "150", 1)
        defer { unsetenv("TORAX_MESH_NCELLS") }

        // Case 1: No CLI override - environment wins
        let reader1 = try await ToraxConfigReader.create(
            jsonPath: configPath,
            cliOverrides: [:]
        )
        ...
        let reader2 = try await ToraxConfigReader.create(
            jsonPath: configPath,
            cliOverrides: cliOverrides
        )
    ...
    func testInvalidJSONFile() async throws {
        await #expect(throws: Error.self) {
            _ = try await ToraxConfigReader.create(
    ...
    func testOverridePriority() async throws {
        ...
        // Set environment variable
        setenv("TORAX_TIME_END", "3.0", 1)
        defer { unsetenv("TORAX_TIME_END") }
